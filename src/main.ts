...
console.log(
  "Добавлен товар в корзину. Товаров в корзине:",
  cartModel.getCount()
);
console.log("Сумма корзины:", cartModel.getTotal());
...
console.log(
  "Ошибки валидации (должно быть 0):",
...
console.log(
  "Количество ошибок:",
  Object.keys(validationErrorsAfterClear).length
);
...
shopApi
  ...
  .then((data) => {
    ...
    console.log(
      "Каталог сохранен в модель. Товаров в модели:",
    ...
    catalogModel
      ...
      .forEach((product, index) => {
        console.log(
          `${index + 1}. ${product.title} - ${product.price} синапсов`
        );
      });
  })
  .catch((error) => {
